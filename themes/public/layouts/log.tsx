...
/**
 * A session's entry in the log (S31, docs/23 §4): a reading page, one scheme, like a post — with the
 * studio's facts strip under the title instead of a byline: the session's own name (`session`, the
 * eyebrow too), the pull request when one carried it, the decisions it added as links into docs/11 §8
 * on GitHub, and what kind of entry it is (the `kind` term). The fields are read by name because the
 * site's `snypd.yaml` declares them on this type; an entry that lacks one shows the cells it has.
 */
...
export default function Log({ ctx, page, adjacent, route, title, description, jsonLd }: LayoutProps): Html {
  ...
  if (session) cells.push({ term: "Session", body: <span>{session}</span> });
  if (pr) cells.push({ term: "Pull request", body: <span><a href={`${PR}${pr}`} rel="external">#{pr}</a></span> });
      ...
      <main>
        <article class="snypd-post snypd-log">
          ...
          <p class="snypd-byline">
            ...
            {p.author ? <> · {p.author.page ? <a href={`${p.author.route}/`} rel="author">{p.author.title}</a> : p.author.title}</> : null}
          </p>
          ...
          {next ? (
            <nav class="snypd-release-nav" aria-labelledby="snypd-next">
